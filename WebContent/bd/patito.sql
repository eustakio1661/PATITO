...
/*FUNCIÃ¯Â¿Â½N AUTOGENERADO DEL CORREO-->EMPLEADO*/
...
/*FUNCIÃ¯Â¿Â½N AUTOGENERADO DEL USUARIO-->EMPLEADO*/
...
/*FUNCIÃ¯Â¿Â½N AUTOGENERADO DE LA CLAVE-->EMPLEADO*/
...
BEGIN
    ...
    CASE
    WHEN SUM(P.CANTIDAD_TOTAL)>1000 THEN 'COMPRADOR FRECUENTE'
    WHEN SUM(P.CANTIDAD_TOTAL)>500  THEN 'COMPRADOR HABITUAL'
...
BEGIN
    SELECT  
    CASE
    WHEN SUM(P.CANTIDAD_TOTAL)>1000 THEN 0.10
    WHEN SUM(P.CANTIDAD_TOTAL)>500  THEN 0.05
    ELSE 0.00
    END AS DESCUENTO
    FROM CLIENTE AS C
...
DROP PROCEDURE IF EXISTS USP_CLIENTEXESTADO;
DELIMITER $$
CREATE PROCEDURE USP_CLIENTEXESTADO(ESTADO_CLI TINYINT)
BEGIN
    SELECT ID_CLI, ID_DIST, DNI_CLI, CONCAT(NOM_CLI, ' ' ,APE_CLI) AS 'NOMBRE CLIENTE', DIR_CLI, TELEF_CLI, ESTADO
    FROM CLIENTE
    WHERE ESTADO = ESTADO_CLI ;
END$$
DELIMITER ;

CALL USP_CLIENTEXESTADO(1);
