...
(
    ...
    ID_CA       INT           NOT NULL,
    IMAGEN      VARCHAR(1000) not NULL DEFAULT 'https://cutt.ly/unbQLrJ',
    ESTADO      TINYINT       NOT NULL DEFAULT 1,
...
RETURNS VARCHAR(50) DETERMINISTIC
	BEGIN
	RETURN CONCAT(CONCAT(LOWER('p'),YEAR(SYSDATE()),DAY(SYSDATE()),FLOOR(RAND()*26)+65,FLOOR(RAND()*26)+97));
    END$$
DELIMITER ;
...
DROP PROCEDURE IF EXISTS USP_BUSCAREMPLEADO;
DELIMITER $$
CREATE PROCEDURE USP_BUSCAREMPLEADO(ID INT)
BEGIN
...
BEGIN
SELECT E.ID_EM,E.DNI,E.NOMBRE,E.APELLIDO,E.TELEFONO,E.CORREO,TE.DESCRIPCION, E.ESTADO FROM EMPLEADO AS E
INNER JOIN TIPOEMPLEADO AS TE
ON E.ID_TP=TE.ID_TP
WHERE E.ESTADO=1 AND TE.ID_TP NOT IN(1);
END$$
DELIMITER ;
...
BEGIN
    SELECT P.ID_PRO, P.DESCRIPCION, P.PRECIO, P.CANTIDAD, P.ID_CA, C.DESCRIPCION, P.IMAGEN, P.ESTADO
    FROM PRODUCTO AS P
...
CALL REGISTRAR_EMPLEADO('89438934','Darick','Leroy','983498345','Calle Honolulu . Mz U Lt. 17',1);
...
INSERT INTO DISTRITO values (8, 'Jesus Maria');
...
INSERT INTO DISTRITO values (25, 'Villa Maria del Triunfo');
...
CALL USP_REGISTRARCLIENTE(4, '87123484', 'Adrian', 'Buleje', 'Av. Arriba 1154', '978621178');
CALL USP_REGISTRARCLIENTE(5, '39874789', 'Marcelo', 'Agachate Conocelo', 'Puno,3721', '943678347');
CALL USP_REGISTRARCLIENTE(6, '98349823', 'Pedro', 'Castillo', 'Av. La Paz 356 Of. 405', '934734433');
CALL USP_REGISTRARCLIENTE(7, '78237832', 'Jose', 'Lurita', 'Av. 1240 Km 12', '984378934');
...
BEGIN
    INSERT INTO PEDIDO VALUES (ID_PE,ID_EM, ID_CLI, sysdate(), CANT,default);
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS USP_REGISTRARDETALLEPEDIDO;
DELIMITER $$
CREATE PROCEDURE USP_REGISTRARDETALLEPEDIDO(ID_PE INT, ID_PRO INT, PRECIO DECIMAL(6, 2),CANTIDAD TINYINT,IMPORTE DECIMAL(6, 2)
)
BEGIN
    INSERT INTO DETALLE_PEDIDO VALUES (ID_PE,ID_PRO,PRECIO,CANTIDAD,IMPORTE);
END$$
DELIMITER ;
...
BEGIN
    ...
    CASE
    WHEN SUM(P.CANTIDAD_TOTAL)>100 THEN 'COMPRADOR FRECUENTE'
    WHEN SUM(P.CANTIDAD_TOTAL)>50  THEN 'COMPRADOR HABITUAL'
...
BEGIN
    SELECT  
    CASE
    WHEN SUM(P.CANTIDAD_TOTAL)>100 THEN 0.10
    WHEN SUM(P.CANTIDAD_TOTAL)>50  THEN 0.05
    ELSE 0.00
    END AS DESCUENTO
    FROM CLIENTE AS C
...
CALL USP_DESCUENTOCLIENTE(2);

DROP PROCEDURE IF EXISTS USP_CLIENTEXDNI;
DELIMITER $$
CREATE PROCEDURE USP_CLIENTEXDNI(DNI CHAR(8))
BEGIN
    SELECT C.ID_CLI,CONCAT(C.NOM_CLI, ' ' ,C.APE_CLI) AS 'NOMBRE CLIENTE', D.NOM_DIST, C.DIR_CLI
    FROM CLIENTE AS C
...
DROP PROCEDURE IF EXISTS USP_listarPedidosPendientes;
DELIMITER $$
CREATE PROCEDURE USP_listarPedidosPendientes()
BEGIN
select p.id_pe, CONCAT(e.nombre, ' ' ,e.apellido) AS EMPLEADO,CONCAT(C.NOM_CLI, ' ' ,C.APE_CLI) AS CLIENTE,p.fecha_pe, p.cantidad_total,p.estado from pedido p 
...
call USP_REGISTRARPEDIDO(1,1,1,100);
call USP_REGISTRARDETALLEPEDIDO(1,5,10.99,100,1099);
call USP_REGISTRARBOLETA(1,'B001-000001',1099,0.00);
--
call USP_REGISTRARPEDIDO(2,1,2,20);
call USP_REGISTRARDETALLEPEDIDO(2,4,42,20,840);
call USP_REGISTRARBOLETA(2,'B001-000002',840,0.00);
--
call USP_REGISTRARPEDIDO(3,1,3,55);
call USP_REGISTRARDETALLEPEDIDO(3,9,10,55,550);
call USP_REGISTRARBOLETA(3,'B001-000003',550,0.00);
--
call USP_REGISTRARPEDIDO(4,1,4,30);
call USP_REGISTRARDETALLEPEDIDO(4,11,5.99,30,71.88);
call USP_REGISTRARBOLETA(4,'B001-000004',71.88,0.00);
--
call USP_REGISTRARPEDIDO(5,1,5,60);
call USP_REGISTRARDETALLEPEDIDO(5,14,17.99,60,1079.4);
call USP_REGISTRARBOLETA(5,'B001-000005',1079.4,0.00);


DROP PROCEDURE IF EXISTS USP_ESTADOEMPLEADO;
DELIMITER $$
CREATE PROCEDURE USP_ESTADOEMPLEADO(ESTADO TINYINT)
BEGIN
SELECT E.ID_EM,E.DNI,E.NOMBRE,E.APELLIDO,E.TELEFONO,E.CORREO,TE.DESCRIPCION,E.ESTADO FROM EMPLEADO AS E
INNER JOIN TIPOEMPLEADO AS TE
ON E.ID_TP=TE.ID_TP
WHERE E.ESTADO=ESTADO AND TE.ID_TP NOT IN(1);
END$$
DELIMITER ;


DROP PROCEDURE IF EXISTS USP_ESTADOPRODUCTOS;
DELIMITER $$
CREATE PROCEDURE USP_ESTADOPRODUCTOS(ESTADO TINYINT)
BEGIN
    SELECT P.ID_PRO, P.DESCRIPCION, P.PRECIO, P.CANTIDAD, P.ID_CA, C.DESCRIPCION, P.IMAGEN,P.ESTADO
    FROM PRODUCTO AS P
             INNER JOIN CATEGORIA C
...
DROP PROCEDURE IF EXISTS USP_ESTADOCLIENTES;
DELIMITER $$
CREATE PROCEDURE USP_ESTADOCLIENTES(ESTADO TINYINT)
BEGIN
    SELECT C.ID_CLI,
           C.DNI_CLI,
           C.NOM_CLI,
